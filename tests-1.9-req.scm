(add-tests-with-string-output "cons, car, cdr"
  [(car (cons 8 32)) => "8\n"]
  [(cons 8 32) => "(8 . 32)\n"]
  [(let [a 8 b 32]
     (cdr (cons a b))) => "32\n"]
...
(add-tests-with-string-output "set-car! set-cdr!"
  ...
  [(let [x (cons 12 13)
     ...
     (set-cdr! x y)
     y) => "(14 . 15)\n"]
  ;;; How should lists be printed?
  [(let [x  (cons 1 2)]
     (set-car! x #t)
     (set-cdr! x #f)
     (cons x x)) => "((#t . #f) #t . #f)\n"]
  ;;; This raises questions about the object model of lists
...
(add-tests-with-string-output "vectors"
  ...
  [(vector? (make-vector 0 nil)) => "#t\n"]
  [(vector-length (make-vector 12 nil)) => "12\n"]
  [(vector-length (make-vector 25 nil)) => "25\n"]
  [(vector? (cons 1 2)) => "#f\n"]
  [(vector? 1287) => "#f\n"]
  [(vector? ()) => "#f\n"]
  [(vector? #t) => "#f\n"]
  [(vector? #f) => "#f\n"]
  [(pair? (make-vector 12 #t)) => "#f\n"]
  [(null? (make-vector 12 #f)) => "#f\n"]
  [(boolean? (make-vector 12 7)) => "#f\n"]
  [(make-vector 0 nil) => "#()\n"]
  [(let (v (make-vector 2 #t))
     (vector-ref-unsafe v 1)) => "#t\n"]
  [(let (v (make-vector 2 #t))
     (vector-ref v 1)) => "#t\n"]
  [(let (v (make-vector 2 nil))
     (vector-set-unsafe! v 0 #t) 
     v) => "#(#t ())\n"]
  [(let (v (make-vector 2 nil))
     (vector-set-unsafe! v 0 #t)
     (vector-set-unsafe! v 1 #f)
     v) => "#(#t #f)\n"]
  [(let (v (make-vector 2 nil))
     (vector-set! v 0 #t)
     (vector-set! v 1 #f)
  ...
  [(let (v (make-vector 1 nil)
         y (cons 1 2))
     (vector-set! v 0 y)
     (cons y (eq? y (vector-ref v 0)))) => "((1 . 2) . #t)\n"]
  [(let (v0 (make-vector 2 nil))
     (let (v1 (make-vector 2 nil))
       (vector-set! v0 0 100)
       (vector-set! v0 1 200)
       (vector-set! v1 0 300)
       (vector-set! v1 1 400)
       (cons v0 v1))) => "(#(100 200) . #(300 400))\n"]
  [(let (v0 (make-vector 3 nil))
     (let (v1 (make-vector 3 nil))
       (vector-set! v0 0 100)
       (vector-set! v0 1 200)
       (vector-set! v0 2 150)
       (vector-set! v1 0 300)
  ...
  [(let (n 2)
     (let (v0 (make-vector n nil))
       (let (v1 (make-vector n nil))
         (vector-set! v0 0 100)
         (vector-set! v0 1 200)
         (vector-set! v1 0 300)
         (vector-set! v1 1 400)
  ...
  [(let (n 3)
     (let (v0 (make-vector n nil))
       (let (v1 (make-vector (vector-length v0) nil))
         ...
         (cons v0 v1)))) => "(#(100 200 150) . #(300 400 350))\n"]
  [(let (n 1)
     (vector-set! (make-vector n nil) (fxsub1 n) (fx* n n))
     n) => "1\n"]
  [(let (n 1)
     (let (v (make-vector 1 nil))
       (vector-set! v (fxsub1 n) n)
       (vector-ref v (fxsub1 n)))) => "1\n"]
  [(let (v0 (make-vector 1 nil))
    (vector-set! v0 0 1)
    (let (v1 (make-vector 1 nil))
      (vector-set! v1 0 13)
